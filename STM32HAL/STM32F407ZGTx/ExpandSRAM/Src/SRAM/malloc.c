...
uint8_t mem2base[MEM2_MAX_SIZE] __attribute__((section(".malloc"))); //外部SRAM内存池
uint8_t mem3base[MEM3_MAX_SIZE] __attribute__((section(".ccmram"))); //内部CMM内存池
//内存管理表
uint16_t mem1mapbase[MEM1_ALLOC_TABLE_SIZE];									    //内部SRAM内存池MAP
uint16_t mem2mapbase[MEM2_ALLOC_TABLE_SIZE] __attribute__((section(".malloc")));	//外部SRAM内存池MAP
uint16_t mem3mapbase[MEM3_ALLOC_TABLE_SIZE] __attribute__((section(".ccmram")));	//内部CCM内存池MAP
//内存管理参数	   
