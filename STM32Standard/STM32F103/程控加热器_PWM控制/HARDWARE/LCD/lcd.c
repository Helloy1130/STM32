...
//size:字体大小
//*p:汉字起始地址		  
void LCD_ShowString_hanzi1(int x,u16 y,u16 width,u16 height,u8 size,char *p)
{         
	int x0 = x;
	u8 num=0,i;
	width+=x;
	height+=y;
    while(*p)		//汉字个数      *P等价于*P!=0  (*P不等于0)
    {    
		num++;
        p++;
	}
	 
	for(i=0;i<(num/2);i++)		//汉字一个一个显示
	{
		if(x>=width)
        {x=x0;y+=size;}
        if(y>=height)break;//退出
        LCD_ShowChar_hanzi1(x,y,i,size,0);
        x+=size;
	}		
}	
...
//size:字体大小 12/16/24
//mode:叠加方式(1)还是非叠加方式(0)
void LCD_ShowChar_hanzi1(int x,u16 y,u8 num,u8 size,u8 mode)
{  							  
    u8 temp,t1,t;
	u16 y0=y;
	u8 csize=size*2;		//得到字体对应点阵集所占的字节数	
// 	num=num-' ';//得到偏移后的值（ASCII字库是从空格开始取模，所以-' '就是对应字符的字库）
	if(x<0) return;	//超过了范围,直接返回
	for(t=0;t<csize;t++)
	{   
		if(size==12) temp=Hanzi_1206[num][t]; 	 	//调用1206字体
		else return  ;								//没有的字库
		for(t1=0;t1<8;t1++)
		{			    
			if(temp&0x80)LCD_Fast_DrawPoint(x,y,POINT_COLOR);
