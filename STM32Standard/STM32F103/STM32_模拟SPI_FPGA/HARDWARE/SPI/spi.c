...
//-----------------------------------------------------------------
void SP2_Send_Data(u16 Send_Data)
{	
	u8	i;
...
void SPI4_Write_Byte(u16 Data) 
{
	...
	{	
		SPI4_CLK_Clr;					 
		if((Data&0x8000) == 0x8000)									// 处理得到一位并发送
		{
			SPI4_MOSI_Set;
		}			
		else
...
//		SPI4_CLK_Clr;
//		
//		if(SPI4_MISO == 0x0001)      							// 按高位在前顺序逐位接收
//		{																				// MISO端数据并缓存在data中
//			data |= 0x0001;
//		}
		
		SPI4_CLK_Set;														// CLK的下降沿MISO端数据有效		
		SPI4_CLK_Clr;												// 下降沿接收数据	
		
		data<<=1;														// 左移	
		data = data | SPI4_MISO;		// 接收数据
	}
	SPI4_CS_Set;
	return data;	            								// 返回2字节数据
}
...
// 注意事项: 无
//-----------------------------------------------------------------
u16 SPI4_Write_Read(u16 Digital)
{
